(ns coalops.store-contract-test
...
(deftest seed-db-read-parity
  (let [s (store/seed-db)]
    (is (= "Northern Coal Mine" (:name (store/site s "coal-mine-1"))))
    (is (true? (:registered? (store/site s "coal-mine-1"))))
    (is (true? (:verified? (store/site s "coal-mine-1"))))
    (is (true? (:registered? (store/site s "coal-mine-3"))))
    (is (false? (:verified? (store/site s "coal-mine-3"))) "seeded as registered but not yet verified")
    (is (nil? (store/site s "no-such-mine")))
    (is (= ["coal-mine-1" "coal-mine-2" "coal-mine-3"] (mapv :site-id (store/all-sites s))))
    (is (= [] (store/ledger s)))
...
(deftest mem-store-honors-explicit-sites-map
  ...
  (testing "an empty sites map means unregistered everywhere"
    (let [s (store/mem-store {})]
      (is (nil? (store/site s "coal-mine-1"))))))

(deftest commit-record-appends-to-coordination-log
  (let [s (store/seed-db)]
    (store/commit-record! s {:op :log-production-record :site-id "coal-mine-1" :value {:tonnage 100}})
    (store/commit-record! s {:op :schedule-maintenance :site-id "coal-mine-1" :value {:equipment "belt"}})
