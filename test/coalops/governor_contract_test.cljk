(ns coalops.governor-contract-test
  "The governor contract as executable end-to-end tests, driven through
  the full langgraph-clj `coalops.operation` StateGraph (intake ->
  advise -> govern -> decide -> commit | hold | request-approval). The
  single invariant under test:

    CoalOpsAdvisor never commits a proposal the CoalMiningGovernor
...
(deftest clean-production-log-auto-commits-at-phase-3
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-record :site-id "coal-mine-1"
                   :patch {:tonnage 4200}} operator-phase-3)]
...
(deftest clean-production-log-needs-approval-at-phase-1
  (testing "phase 1 has an empty :auto set -- every write escalates for human approval, even when clean"
    (let [[db actor] (fresh)
          res (exec-op actor "t2" {:op :log-production-record :site-id "coal-mine-1"
...
(deftest maintenance-and-shipment-auto-commit-clean-at-phase-3
  (let [[db actor] (fresh)]
    (exec-op actor "t3a" {:op :schedule-maintenance :site-id "coal-mine-1" :patch {:equipment "belt"}} operator-phase-3)
    (exec-op actor "t3b" {:op :coordinate-shipment :site-id "coal-mine-1" :patch {:carrier "rail-1"}} operator-phase-3)
...
(deftest safety-concern-always-escalates-then-human-decides
  (testing "a clean, high-confidence safety-concern flag still ALWAYS interrupts for human sign-off -- never auto, at any phase"
    (let [[db actor] (fresh)
          r1 (exec-op actor "t4" {:op :flag-safety-concern :site-id "coal-mine-1"
                                  :patch {:concern "elevated methane" :confidence 0.99}} operator-phase-3)]
...
(deftest safety-concern-rejected-by-human-is-held-not-committed
  (let [[db actor] (fresh)
        _ (exec-op actor "t5" {:op :flag-safety-concern :site-id "coal-mine-1"
...
(deftest unregistered-site-is-held-and-unoverridable
  (testing "an unregistered site -> HOLD, settles immediately, never reaches request-approval"
    (let [[db actor] (fresh)
          res (exec-op actor "t6" {:op :log-production-record :site-id "coal-mine-9"
...
(deftest unverified-site-is-held
  (testing "coal-mine-3 exists but is registered? true / verified? false -> HOLD"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :log-production-record :site-id "coal-mine-3"
                                   :patch {:tonnage 10}} operator-phase-3)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:site-unverified} (-> (store/ledger db) first :basis)))
      (is (= [] (store/coordination-log db))))))
...
(deftest direct-actuation-effect-is-held-and-unoverridable
  (testing "an advisor that drafts a non-:propose :effect is HARD-blocked, never reaches request-approval"
    (let [[db _actor] (fresh)
          ...
          res (exec-op actor2 "t8" {:op :coordinate-shipment :site-id "coal-mine-1"
...
(deftest scope-excluded-proposal-is-held-and-permanent
  (testing "a proposal that drifts into blasting/extraction-sequencing scope -> HARD hold, never reaches request-approval, at ANY confidence"
    (let [[db actor] (fresh)
          res (exec-op actor "t9" {:op :schedule-maintenance :site-id "coal-mine-1"
...
(deftest op-outside-allowlist-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :extract-material :site-id "coal-mine-1"
...
(deftest every-decision-leaves-one-ledger-fact
  (testing "write-only-through-ledger: N operations -> N ledger facts"
    (let [[db actor] (fresh)]
      (exec-op actor "a" {:op :log-production-record :site-id "coal-mine-1" :patch {:tonnage 1}} operator-phase-3)
      (exec-op actor "b" {:op :log-production-record :site-id "coal-mine-9" :patch {:tonnage 1}} operator-phase-3)
      (is (= 2 (count (store/ledger db)))
