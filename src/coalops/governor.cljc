(ns coalops.governor
  "CoalMiningGovernor -- the independent compliance layer that earns
  the CoalOpsAdvisor the right to commit. The advisor has no notion of
  whether a mine/site is actually registered and verified, whether its
  own proposed `:effect` secretly claims a direct actuation instead of
  a mere proposal, or whether it has silently drifted into a
  ...
  or authorizes:
    - direct extraction sequencing (blasting order, drilling patterns,
      cutting schedules)
    - mine-safety decisions (ventilation control, gas-monitoring
      overrides, structural-stability determinations)
  ...
  `coalops.phase` independently agrees: `:flag-safety-concern` is never
...
(def scope-excluded-terms
  ...
  ["blast" "発破"
   ...
   "ventilation control" "ventilation-control" "換気制御"
   "gas-monitoring override" "gas monitoring override" "ガス監視制御"
   "structural-stability decision" "structural stability decision"
...
(defn- scope-exclusion-violations
  ...
  (let [op (:op proposal)
    ...
    (cond
      ...
      [{:rule :scope-excluded
        :detail "採掘順序/発破/ドリリング・カッティング計画/換気・ガス監視制御/鉱山保安機関の判断領域に触れる提案は永久に禁止"}])))

(defn check
  "Censors a CoalOpsAdvisor proposal against the governor rules.
