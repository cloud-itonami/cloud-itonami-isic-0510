(ns coalops.advisor
  "CoalOpsAdvisor -- the *contained intelligence node* for the ISIC-0510
  hard-coal-mining operations-coordination actor.
  ...
  by `coalops.governor` before anything touches the SSoT.

  This advisor NEVER drafts extraction sequencing, blasting/drilling/
  cutting schedules, ventilation/gas-monitoring control, or any
  mine-safety-authority decision (permit issuance, license suspension,
  compliance enforcement) -- those are permanently out of scope for
  this actor, not merely un-implemented. `coalops.governor`'s
...
(defn- propose-safety-concern
  "Surface a mine-safety concern (gas, structural, ventilation) for
  HUMAN triage. This op ALWAYS escalates in `coalops.governor` --
  never auto-committed at any phase (`coalops.phase`) -- regardless of
  how confident the advisor is that the concern is real or minor. The
  advisor itself makes NO safety determination; it only surfaces the
  observation."
  [_db {:keys [site-id patch]}]
  {:op         :flag-safety-concern
   :site-id    site-id
   :summary    (str site-id " の安全上の懸念を提起: " (pr-str (keys patch)))
   :rationale  "観測された懸念事象の提起のみ。安全性の評価・是正措置の決定は行わない -- 常に人間審査が必要。"
   :cites      [site-id]
   :effect     :propose
   :value      (merge {:site-id site-id} patch)
...
(defn- propose-shipment
  "Draft outbound coal-shipment coordination (loadout scheduling,
...
(defn- propose-out-of-scope
  ...
  {:op         :schedule-maintenance
   ...
   :rationale  "次回のdrilling patternとblast sequenceを調整済み"
   :cites      [site-id]
   :effect     :propose
   :value      (merge {:site-id site-id} patch)
   :confidence 0.9})
...
(def ^:private system-prompt
  (str "あなたは石炭鉱山の運営コーディネーション助言者です。"
       ...
       "採掘順序・発破・ドリリング/カッティング計画・換気/ガス監視制御・"
       "鉱山保安機関の判断(許可発行/免許停止/コンプライアンス執行)には"
       "絶対に触れてはいけません。"
