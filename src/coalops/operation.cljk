(ns coalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (CoalOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CoalMiningGovernor (:govern)
  ...
  (:require [langgraph.graph :as g]
            ...
            [coalops.governor :as governor]
            [coalops.phase :as phase]
...
(defn build
  ...
  `coalops.store/Store`).
  opts:
    :advisor      -- a `coalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CoalOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; CoalMiningGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
