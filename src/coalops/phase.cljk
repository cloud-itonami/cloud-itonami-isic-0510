(ns coalops.phase
  "Phase 0->3 staged rollout for the ISIC-0510 hard-coal-mining
  ...
  needs a human to actually look at it. `coalops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [coalops.governor :as governor]))
...
(defn verdict->disposition
  "Map a CoalMiningGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
